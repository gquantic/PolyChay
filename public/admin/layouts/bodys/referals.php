<?php
$refers = $Db->query("SELECT * FROM `refers`");
?>
...
<div class="row">
    <div class="col-md-12 col-lg-12 col-xl-10">
        ...
        <table class="table" style="width: 100%;">
            ...
            <tbody>
            <? $linkId = 0; while($refer = mysqli_fetch_assoc($refers)): $linkId++;
                ?>
                <tr>
                    <td><?= $refer['name'] ?></td>
                    <td><?= $refer['city'] ?></td>
                    <td><?= $refer['status'] ?></td>
                    <td>
                        <span class="link"><?= "http://polychay.ru/referalReg.php?id=".$refer['id'] ?></span>
                        <a href="javascript:copyLink('#link-<?= $linkId ?>');">
                            <svg width="35px" height="35px"><use transform="scale(.6) translate(5, 10)" xlink:href="#icon-paper-clip"></use></svg>
                        </a>
                        <input class="link display-inline" style="display: inline;position: absolute;z-index: -1;opacity: 0;" id="link-<?= $linkId ?>" value="<?= "http://polychay.ru/referalReg.php?id=".$refer['id'] ?>">
                    </td>
