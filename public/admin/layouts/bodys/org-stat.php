...
<!--                        <th style="width:50px;"></th>-->
                    ...
                    <? while($user = mysqli_fetch_assoc($users)):
                        $rating = $Db->fQuery("SELECT AVG(`stars`) AS midlestars FROM `feedbacks` WHERE `employer`='{$user['id']}'");
                        $sum = $Db->fQuery("SELECT SUM(`amount`) AS amount FROM `transacts` WHERE `employeer`='{$user['id']}'");
                        ?>
                        <tr style="">
                            <td><a href="userstatistic?id=<?=$user['id'];?>"><?=$user['name'];?></a></td>
                            <td><?=$user['balance']?> р.</td>
                            <td><svg
                                ...
                                </svg> <?php if (strlen($rating['midlestars']) == 1) echo $rating['midlestars'].",0"; else echo $rating['midlestars']; ?></td>
                            <td><?= $sum['amount'] ?> р.</td>
<!--                            <td><a href=""><svg height="25px" width="25px"><use xlink:href="#icon-pencil"></use></svg></a></td>-->
                        </tr>
                    <? endwhile; ?>
                    <?php $Db->checkNoEmptyTable($users, 4); ?>
